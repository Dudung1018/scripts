...
@Author：怎么肥事（Surge适配整理）

[Script]
PingMe-Capture = type=http-request, pattern=^https:\/\/api\.pingmeapp\.net\/app\/queryBalanceAndBonus, script-path=PingMe.js, requires-body=false
PingMe-Task = type=cron, cronexp="30 8,20 * * *", script-path=PingMe.js
  ...
  if (!raw) {
    ...
    async function doVideoLoop(count) {
      for (let i = 1; i <= count; i++) {
        const delay = i === 1 ? 1500 : VIDEO_DELAY;
        log(`视频任务 ${i}，等待 ${delay}ms`);
        await wait(delay);

        try {
          const res = await fetchApi('videoBonus');
          log(`videoBonus ${i} 返回: ${res.statusCode}`);
          ...
          if (d.retcode === 0) {
            ...
            msgs.push(`⏸ 视频${i}：${d.retmsg || '任务结束'}`);
            break;
          }
        } catch (err) {
          msgs.push(`❌ 视频${i}：${err.error || err.message || String(err)}`);
          break;
        }
      }
    }
    ...
    (async () => {
      try {
        ...
        log(`任务异常: ${err && err.stack ? err.stack : String(err)}`);
